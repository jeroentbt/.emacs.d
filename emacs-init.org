  ...
  (use-package modus-themes
    ...
    (setq modus-themes-italic-constructs t
          ...
          '((1 . (overline variable-pitch background 1.3))
            ...
            (4 . (overline 1.15))
            (t . (semibold 1.1))))
    ;; Load the themes without enabling
    (load-theme 'modus-videndi t t)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package smart-mode-line
               ...
               :config
                   ...
                   (setq sml/extra-filler -6) ;; bugfix
                   ;; Set min and max buffer name length
                   (setq sml/name-width '(12 . 44))
                   (setq sml/shorten-modes t)
                   (setq sml/shorten-directory t))
#+END_SRC
***** WAITING truncation bugfix                                     :bug:
see [[https://github.com/Malabarba/smart-mode-line/issues/261][Modeline trucation in Emacs 29 · Issue #261 · Malabarba/smart-mode-line]]
fixed for now by adding ~(setq sml/extra-filler -6)~ in the code above

***** Diminish minor modes that are not explicitly loaded anywhere else
#+begin_src emacs-lisp :tangle yes
...
***** window resizing
****** implemented with Transient                             :disabled:
...
Also (and always) add some margin using a default variable
...
***** Tramp and zsh do not play nice
#+begin_src emacs-lisp :tangle yes
...
The compilation buffer pops up in a new window in whatever frame you are in.
With this snippet emacs will consider any existing frame with the buffer open
#+begin_src emacs-lisp :tangle yes
  (push '("\\*compilation\\*"
          :position bottom
          :height 0.25) ; Adjust the percentage as needed
        popwin:special-display-config)
  (push '("\\*compilation\\*"
...
*** TODO Tree-sitter (buffer-local up-to-date syntax tree)
...
Disabled for now, see how to install on: https://github.com/mickeynp/combobulate
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package js2-mode
               ...
               "\\.js\\'"
               :config
               (progn (add-hook 'js2-mode-hook (lambda () (tern-mode t)))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (setq org-capture-templates
        (quote (("t" "TODO" entry (file (lambda () (concat org-directory "/refile.org")))
                 "* TODO %? %^G\n  %U\n")
                ("r" "respond" entry (file (lambda () (concat org-directory "/refile.org")))
                 "* NEXT Respond to %:from on %:subject\nSCHEDULED: %t\n%U\n%a\n")
                ("n" "note" entry (file (lambda () (concat org-directory "/refile.org" )))
                 "* %? :NOTE:\n%U\n%a\n")
                ("a" "interesting app" entry (file (lambda () (concat org-directory "/interesting_apps.org")))
                 "* %? \n")
                ("j" "journal entry" entry (function org-journal-find-location)
                 "** %(format-time-string org-journal-time-format)%^{Title}\n%i%?")
                ("s" "org-protocol selection" entry (file (lambda () (concat org-directory "/refile.org")))
                 "* %?\n  %a\n  %i")
                ("l" "org-protocol link" entry (file (lambda () (concat org-directory "/refile.org")))
                 "* %?\n  %a")
                ("m" "Meeting" entry (file (lambda () (concat org-directory "/refile.org")))
                 "* MEETING with %? :MEETING:\n%U")
                ("p" "Phone call" entry (file (lambda () (concat org-directory "/refile.org")))
                 "* PHONE %? :PHONE:\n%U")
                ("h" "Habit" entry (file (lambda () (concat org-directory "/refile.org")))
...
***** Capture web addresses from your browser by adding this bookmarklet
...
To disable the dialogue box asking if it is ok to open using org-protocol
in firefox ~about:config~ add: ~network.protocol-handler.external.org-protocol~ and set it to ~True~
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  ;; rename files when moving to support mbsync
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'mu4e-bookmarks
               '( :name  "Unread - lists"
                  :query "flag:unread AND flag:list AND NOT flag:trashed AND NOT maildir:/.*/Trash/"
                  :key   ?l))
  (add-to-list 'mu4e-bookmarks
               '( :name  "Unread - no lists, today"
                  :query "flag:unread and date:today..now AND NOT flag:trashed AND NOT maildir:/.*/Spam/"
                  :key   ?d))
  (add-to-list 'mu4e-bookmarks
               '( :name  "Unread - no lists"
                  :query "flag:unread AND NOT flag:list AND NOT flag:trashed AND NOT maildir:/.*/Spam/"
                  :key   ?b))
  (add-to-list 'mu4e-bookmarks
               '( :name  "Flagged"
                  :query "flag:flagged"
...
******* a cron job to delete old messages from trash
Once I fix trashing...
#+begin_src shell
mu find maildir:/mailbox/Trash AND date:40y..90d --sortfield=date --fields=l | xargs mu remove
#+end_src


****** Find messages by same sender
 #+begin_src emacs-lisp :tangle yes
   (add-to-list 'mu4e-view-actions
                  '("find-sender-unread" . mu4e-action-find-sender-unread) t)
   (add-to-list 'mu4e-headers-actions
                  '("find-sender-unread" . mu4e-action-find-sender-unread) t)

   (defun mu4e-action-find-sender-unread (msg)
     "Find all unread messages from the same sender as the current message."
     (let* ((sender-info (mu4e-message-field msg :from))
            (sender-email (plist-get (car sender-info) :email )))
       (mu4e-headers-search (concat "flag:unread from:" (regexp-quote sender-email)))))
 #+end_src
...
Using [[https://github.com/kljohann/mpv.el][kljohann/mpv.el: control mpv for easy note taking]]
...
#+begin_src emacs-lisp :tangle yes
  (use-package mpv
    ...
    ;; insert timestamp of video as org-timer-item
    ...
    (setq mpv-default-options '("--script-opts=osc-visibility=always")))
#+end_src
...
** profiler
~M-x profiler-start~ will start a profiler sessions showing you where emacs spent its time

** Kick emacs
This script is on my path as ~kick_emacs~

#+begin_src shell
  #!/bin/sh

  # when emacs hangs, sending SIGUSR2 can kick it back in to reality
  # this sets debug-on-quit, which is why we immediatly undo it
