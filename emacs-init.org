...
#+BEGIN_SRC emacs-lisp :tangle yes
  (req-package helm-dash
    ...
    (setq helm-dash-browser-func nil)
    :bind
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq org-babel-load-languages '((js . t)
                                   (sh . t)
                                   (org . t)
                                   (python . t)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
    (req-package helm-config
      :require (helm async helm-ag)
      :bind ("C-c h" . helm-mini)
      :config ((setq helm-autoresize-max-height 40
...
** 2015-01-16 gpg 2.1 eps-list-keys bug
Prevents using gpg keys in emacs to send mail, list keys etc
see https://bbs.archlinux.org/viewtopic.php?id=190497

solution, replace the existing code to ignore extra arguments:
